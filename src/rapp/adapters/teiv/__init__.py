...
populate :class:`~rapp.domain.topology.NetworkTopology`, then subscribe to
O1 VES ``cellStatusChange`` events (via :class:`~rapp.adapters.ves.VesEventAdapter`)
to keep it current.

rApps should not hard-code cell IDs.  Instead they query topology at startup
and iterate over :meth:`~rapp.domain.topology.NetworkTopology.active_cells`.
...
from core.models.parameters import NodeType
from rapp.domain.topology import NetworkTopology, NodeInfo
...
class TEIVAdapter:
    ...
    def load_topology(self) -> NetworkTopology:
        ...
        try:
            resp = self._session.get(
                f"{self._base}{self._CELLS_PATH}", timeout=10
            )
    ...
    def refresh_node(self, topology: NetworkTopology, cell_id: str) -> None:
        ...
        try:
            resp = self._session.get(
                f"{self._base}{self._CELLS_PATH}/{cell_id}", timeout=10
            )
            resp.raise_for_status()
        except requests.RequestException as exc:
