...
Adapter role
    Translates BMW Lab's internal :class:`core.models.PolicyDecision` (the
    output of an :class:`core.strategies.OptimizationStrategy`) to
    O-RAN-standardised A1 policy JSON, and vice-versa for policy-status
    responses.

    .. code-block:: none

        [core.models.PolicyDecision] <── A1Adapter ──> [O-RAN A1 policy JSON]
...
See also
    :class:`rapp.application.ports.A1Port` — structural typing Protocol used
...
#: Maps a :class:`~core.models.PolicyDecision` to the A1 traffic-steering
...
class A1Adapter:
    ...
    The adapter translates a :class:`~core.models.PolicyDecision` into the
    ...
    def _decision_to_policy_data(cell_id: str, decision: PolicyDecision) -> dict[str, Any]:
        """Map a :class:`~core.models.PolicyDecision` to ``ORAN_TrafficSteering_0.1.0`` data.
...
class A1Error(RuntimeError):
    """Raised when an A1 interface operation fails."""
