...
class Settings:
    ...
    :param platform: ``RAPP_PLATFORM``; default ``mock`` — Abstract Factory
        selector (``mock`` / ``osc`` / ``physical`` / ``ericsson`` / ``nokia``).
        Vendor values select a proprietary management-plane factory.
    ...
    :param ems_base_url: ``RAPP_EMS_BASE_URL`` — vendor management-plane (ENM /
        NetAct) PM API base URL (``ericsson`` / ``nokia`` mode).
    ...
    def from_env() -> Settings:
        ...
        return Settings(
            ...
            ems_base_url=os.getenv("RAPP_EMS_BASE_URL", ""),
