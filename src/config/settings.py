...
class Settings:
    ...
    :param platform: ``RAPP_PLATFORM``; default ``osc`` — Abstract Factory
        selector (``osc`` / ``ns3`` / ``viavi`` / ``oai`` / ``ocudu``).
        Vendor values select the matching KPI-name Adapter; all platforms
        share the OSC O-RAN transport.
    ...
    :param sme_base_url: ``RAPP_SME_BASE_URL`` — Non-RT RIC SME base URL.
    :param ics_base_url: ``RAPP_ICS_BASE_URL`` — Non-RT RIC ICS base URL.
    :param callback_url: ``RAPP_CALLBACK_URL`` — this rApp's callback URL.
    :param cell_id: ``RAPP_CELL_ID``; default ``cell-0`` — primary cell ID.
    ...
    def from_env() -> Settings:
        ...
        platform = os.getenv("RAPP_PLATFORM", "osc").lower()
